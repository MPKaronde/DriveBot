...
NOTE: most of the methods attempt to move by a fixed number of mm and/or
degrees. Although the speed can be trusted, the distance measurements are merely
an ESTIMATE and NOT TO BE FULLY TRUSTED. Although the bot is run via steppers,
      variations such as voltage level, wheel compression, otherwise unknown
drag, etc, can cause the system to drift from these measurements so other
onboard sensors (distance sensors, camera) should be used with this & these
distance should absolutely NOT be applied through Dead Reckoning
...
const double TICKS_PER_CM      = 12;   // # ticks per cm traveled in straight line actual val = 11.16876794
...
bool         SERIAL_RUNNING    = true; // whether to run serial loop. false will need arduino reset to set true
...
// Input: Speed (motor speed) & Distance (ticks) desired to drive in straight
// line Behavior: Robot drives at given speed for given distance in straight
// line Return: True if valid values, False if invalid values or error deteced
...
// Behavior: Rotate in place by given amount at given speed (runs both motors
// opposite directions for required time at given speed) Return: True if valid
// values, False if invalid values or error deteced
...
// Return: true if command success, false if command failure or command not
// recognized
...
// Input: none
// Behavior: continues interpreting and communicating over serial / running
// machine as long as SERIAL_RUNNING is true Return: none
void serial_runner() {
